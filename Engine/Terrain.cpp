...
std::unique_ptr<TTerrainMesh> CTerrainGenerator::GenerateMesh() const
{
  ...
  for (int z = 1; z < vertexCountPerSide - 1; z++)
  {
    for (int x = 1; x < vertexCountPerSide - 1; x++)
    {
...
float CTerrain::GetHeight(glm::vec2 position)
{
  ...
  const int vertexCountPerSide = 500 + 1;

  float gridX = position.x / squareSize;
  float gridZ = position.y / squareSize;

  int x = (int)floor(gridX);
  int z = (int)floor(gridZ);

  if (x < 0 || z < 0 || x >= vertexCountPerSide - 1 || z >= vertexCountPerSide - 1)
    return 0.0f;
