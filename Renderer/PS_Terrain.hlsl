...
float4 PSMain(PS_INPUT input) : SV_TARGET
{
  ...
  float4 colorPeek    = TexturePeek   .Sample(ObjSamplerState, input.TexCoord);
  ...
  // Schnee an den Spitzen
  float snowStart = 10.0f;
  float snowEnd = 20.0f;

  float snowMask = smoothstep(snowStart, snowEnd, input.WorldPos.y);

  float flatness = saturate(input.normal.y);
  snowMask *= flatness;
  
  float4 texSnow = TexturePeek.Sample(ObjSamplerState, input.TexCoord * 2.0f);

  diffuse = lerp(diffuse, colorPeek.rgb, snowMask);
